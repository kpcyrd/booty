...
iso_name=booty
iso_label="BOOTY_$(date +%Y%m)"
iso_publisher="booty <https://github.com/kpcyrd/booty>"
iso_application="booty live/forensic cd"
...
make_customize_airootfs() {
    ...
    if [ -z ${ARCH_MIRROR+x} ]; then
        ...
        echo "Server = $ARCH_MIRROR/\$repo/os/\$arch" > ${work_dir}/x86_64/airootfs/etc/pacman.d/mirrorlist
    fi
    ...
    mkdir "${work_dir}/x86_64/airootfs/root/.ssh"
    chmod 0700 "${work_dir}/x86_64/airootfs/root/.ssh"
    cp --no-preserve=owner "${script_path}/authorized_keys" "${work_dir}/x86_64/airootfs/root/.ssh/authorized_keys"
    chmod 0600 "${work_dir}/x86_64/airootfs/root/.ssh/authorized_keys"

    cp --no-preserve=owner "${script_path}/booty.conf" "${work_dir}/x86_64/airootfs/root/booty.conf"

    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs.sh' run
    rm ${work_dir}/x86_64/airootfs/root/customize_airootfs.sh
    rm ${work_dir}/x86_64/airootfs/root/booty.conf
}
